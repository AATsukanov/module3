...
['Able', 'Disable']
Примечания:
При проверке наичлия одного слова в составе другого стоит учесть, что регистр символов не должен влять ни на что. ('Disablement' - 'Able') ('Able', 'able', 'AbLe' - все подходят)
В этой задаче вам могут понадобиться следующие методы строк/ключевые слова:
    а. Оператор in или count()
    b. lower()/upper().

Файл с кодом (module_2_6.py) прикрепите к домашнему заданию или пришлите ссылку на ваш GitHub репозиторий с файлом решения.
'''
...
def single_root_words(root_word, *other_words):
    ...
    for w in other_words:
        if rw in w.upper() or w.upper() in rw:
            same_words.append(w)
    return same_words
